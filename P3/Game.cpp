...
void Game::drawBoard()
{
	...
	for (int row = 0; row < _board.getRowCount(); row++)
	{
		for (int col = 0; col < _board.getColCount(); col++)
		{
			...
			if (_board.getIfDebug() && _board.getTileState(row, col).isMine())
			{
				sprite.setTexture(_tm.getTexture("mine"));
				sprite.setPosition(col * 32, row * 32);
				_window.draw(sprite);
			...
			if (_board.getIfGameLost() && _board.getTileState(row, col).isMine())
			{
				sprite.setTexture(_tm.getTexture("mine"));
				sprite.setPosition(col * 32, row * 32);
				_window.draw(sprite);
		...
		while (_window.pollEvent(_event))
		{
			...
			else if (_event.type == sf::Event::MouseButtonPressed)
			{
				...
				else
				{
					...
					int test1XPos = _board.getColCount() * 32 - 192;
					int test1YPos = _board.getRowCount() * 32;
					if (isInside(_event.mouseButton.x, _event.mouseButton.y, test1XPos, test1YPos, 64, 64))
					{
						_board.loadBoardFile("boards/testboard1.brd");
					}

					// test2 button
					int test2XPos = _board.getColCount() * 32 - 128;
					int test2YPos = _board.getRowCount() * 32;
					if (isInside(_event.mouseButton.x, _event.mouseButton.y, test2XPos, test2YPos, 64, 64))
					{
						_board.loadBoardFile("boards/testboard2.brd");
					}

					// test3 button
					int test3XPos = _board.getColCount() * 32 - 64;
					int test3YPos = _board.getRowCount() * 32;
					if (isInside(_event.mouseButton.x, _event.mouseButton.y, test3XPos, test3YPos, 64, 64))
					{
						_board.loadBoardFile("boards/testboard3.brd");
					}
				}
			}
		}
